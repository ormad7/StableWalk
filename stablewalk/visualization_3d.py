...
def orient_skeleton_for_display(skeleton: Skeleton3D) -> Skeleton3D:
    """
    Rotate skeleton so the person stands upright (head +Y) and faces the camera.
    Fixes horizontal / face-down figures in the 3D view.
    """
    from stablewalk.skeleton_3d_model import transform_skeleton
    ...
    r1 = _rotation_matrix(spine, np.array([0.0, 1.0, 0.0]))
    sk = transform_skeleton(skeleton, r1.tolist())
    j2 = sk.joints
...
def _draw_body_glow_outline(ax: Axes, skeleton: Skeleton3D, height: float) -> None:
    ...
    from stablewalk.skeleton_3d_model import SKELETON_3D_JOINTS
...
def draw_dof_motion_chart(
    ...
    *,
    ...
    from stablewalk.gait_dof import DOF_LABELS, GAIT_ANGLE_FIELDS
