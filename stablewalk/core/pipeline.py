...
def _run_legacy_two_pass(
    ...
    *,
...
) -> tuple[FrameExtractionResult, list[str]]:
    ...
    return extraction, extraction.frame_paths
...
def run_gait_pipeline(
    ...
    *,
    ...
    if cache_frames == "legacy":
        extraction, frame_paths = _run_legacy_two_pass(
            source,
            frames_dir,
            poses_path,
            from_url=from_url,
        ...
        extraction, frame_paths = _run_single_pass(
            source,
            frames_dir,
            poses_path,
    ...
    logger.info(
        "Loaded new video: %s | run=%s | frames=%d | %dx%d | first_frame=%s",
        source[:80],
        name,
    ...
    if export_paths:
        logger.info(
            "OpenSim export complete for run=%s (MediaPipe source)", name
        )
